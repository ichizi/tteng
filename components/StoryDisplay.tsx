...
export const StoryDisplay: React.FC<StoryDisplayProps> = ({ story, onReset, imageUrl }) => {
  ...
  // --- LAZY LOADING ONLY ---
  // Audio is now fetched on-demand when user clicks the play button
  // This prevents hitting API rate limits by avoiding concurrent bulk requests
  ...
  const handleSpeak = async (text: string, index: number) => {
    ...
    try {
      ...
      if (e.message?.includes('429') || e.toString().includes('Quota') || e.toString().includes('Too Many Requests')) {
        setAudioError("⏳ Quota exceeded, please wait a few seconds (Rate Limit)");
      } else {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto pb-20">
      ...
      {/* Error Toast */}
      {audioError && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-red-500 text-white px-6 py-3 rounded-full shadow-xl z-50 animate-bounce flex items-center gap-2">
